...
const stylesheetHash = md5File.sync(
    path.join(__dirname, "../dist/css/main.css")
);
...
Twig.renderFile(MAIN_TWIG_FILE, twigVariables, (err, html) => {
    ...
    fs.writeFile(OUTPUT_FILE, transformedHTML, err => {
        err
            ? console.log("Could not write the twig/html file to disk")
            : console.log(`Sucessfully compiled ${MAIN_TWIG_FILE}`);
    });
});
